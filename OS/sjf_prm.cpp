...
void arrived(vector<process> &temp,vector<process> &arrived_process,int time){
    ...
    for(int i=0;i<temp.size();i++){
            if(time>=temp[i].arrival_time && temp[i].remaining_time>0){
                p=temp[i];
...
void check_prempt(vector<process> &arrived_process,vector<process> &temp,vector<track> &tracks, int &time){
    track y;
    if(arrived_process.size()>1)
    {
    sort(arrived_process.begin()+1,arrived_process.end(),process_burst_comparator);
        if(arrived_process[0].remaining_time+time> arrived_process[1].arrival_time && (arrived_process[0].burst_time-(arrived_process[1].arrival_time-time))> arrived_process[1].remaining_time ){
            y.p_name=arrived_process[0].p_name;
            y.start_time=time;
            y.end_time=arrived_process[1].arrival_time;
            tracks.push_back(y);
            arrived_process[0].remaining_time=arrived_process[0].burst_time-(arrived_process[1].arrival_time-time);
            time=arrived_process[1].arrival_time;
        ...
        //cout<<"Hello4"<<endl;
            ...
            time+=arrived_process[0].remaining_time;
            arrived_process[0].remaining_time=0;
            //cout<<"time: after release of "<<arrived_process[0].p_name<<"  = "<<time<<endl;
            
            auto it2=temp.begin();
            while(it2!=temp.end()){
                if((*it2).p_name==arrived_process[0].p_name){
                  // cout<<"temp 0 before "<<temp[0].p_name<<endl;
                    temp.erase(it2);
                   //cout<<"temp 0 "<<temp[0].p_name<<endl;
                    break;
            ...
            // cout<<"hello"<<endl;
            // cout<<arrived_process.size()<<endl;
    }
}