...
bool time_process_comparator(process a,process b);
...
int main(){
   int i,j,n,time=0;
    ...
    while(!temp.empty()){
        arrived_sorted(arrived_process,temp,time);
...
void arrived_sorted(vector<process> &arrived_process,vector<process> temp,int time){
    ...
    sort(arrived_process.begin(),arrived_process.end(),time_process_comparator);
}
void read_input(vector<process> &processes, int &n){
    int i;
